...
void CManualDlg::OnDestroy() 
{
	...
	m_pManualGoodNGPickerDlg->DestroyWindow();
	...
	if (m_pManualGoodNGPickerDlg) delete m_pManualGoodNGPickerDlg;
...
void CManualDlg::OnShowWindow(BOOL bShow, UINT nStatus) 
{
	...
	if (bShow) {
		...
		if (m_rdoManualElevator2.GetCheck()) m_pManualUnloadPickerDlg->ShowWindow(SW_SHOW);
		if (m_rdoManualGripper.GetCheck()) m_pManualInspectorDlg->ShowWindow(SW_SHOW);
		if (m_rdoManualPicker.GetCheck()) m_pManualGoodNGPickerDlg->ShowWindow(SW_SHOW);
...
//		m_rdoManualDoorUnlock.Set_Color(RGB(0x00, 0x00, 0xFF), COLOR_DEFAULT);
		...
		if (m_rdoManualElevator2.GetCheck()) m_pManualUnloadPickerDlg->ShowWindow(SW_HIDE);
		if (m_rdoManualGripper.GetCheck()) m_pManualInspectorDlg->ShowWindow(SW_HIDE);

		if (m_rdoManualPicker.GetCheck()) m_pManualGoodNGPickerDlg->ShowWindow(SW_HIDE);
...
void CManualDlg::OnTimer(UINT nIDEvent) 
{
	...
	} else if (m_pManualGoodNGPickerDlg->IsWindowVisible()) {
		m_pManualGoodNGPickerDlg->Display_Status();
...
{	
	...
	m_pManualGoodNGPickerDlg->ShowWindow(SW_HIDE);
