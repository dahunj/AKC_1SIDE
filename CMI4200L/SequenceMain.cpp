...
int CSequenceMain::Get_IsAutoRun(int nType)
{
	...
	for (int i = 0; i < 12; i++) {
		...
		if (i>=0 && i<=3) {		//Load, Unload Tray
			if (nCase[i] > 100 && nCase[i] != 250 && nCase[i] != 200 && nCase[i] != 300 && nCase[i] != 440 && nCase[i] != 450 ) return 1;
		} 
		else {
...
//				if (!m_pCommon->Delay_LoopTime(AUTO_UNLOAD1, 3000)) break;
	...
	case 140:
		if (m_pCommon->Check_Position(AX_UNLOAD_TRAY_Z1, 3)) {
			m_pDY6->oEnd_LoadSupport12In = TRUE;//m_pDY6->oEnd_UnloadSupport12In = TRUE;
			m_pDY6->oEnd_LoadSupport12Out = FALSE;//m_pDY6->oEnd_UnloadSupport12Out = FALSE;
			m_sLog.Format("m_nUnload1Case,%d support In",m_nUnload1Case); pLogFile->Save_MCCLog(m_sLog);
			m_pAJinAXL->Write_Output(6);
			m_nUnload1Case = 150;
			m_pCommon->Set_LoopTime(AUTO_UNLOAD1, 5000);
		}
	...
	case 200:
		if ((m_nUnload2Case < 192 || m_nUnload2Case > 411) && m_pCommon->Check_Position(AX_UNLOAD_TRAY_Y1, 1)) {
	...
	case 350:
		if (!m_pCommon->Check_Position(AX_UNLOAD_TRAY_Y1, 3) || m_pCommon->Check_Position(AX_UNLOAD_TRAY_Y1, 3)) {
			...
			dPosY1 = pStatus->dPos;
			double dPos = m_pMoveData->dUnloadTrayY1[2]; //job position 
			if (dPosY1 >= dPos) {
				gData.nTrayPos[3] = 0;
		...
		break;
	case 351:
		if (m_pDX6->iEnd_Unload1FCheck)
		{
			m_sLog.Format("m_nUnload1Case,%d, %d",m_nUnload1Case,m_pDX6->iEnd_Unload1FCheck); pLogFile->Save_MCCLog(m_sLog);
			break;
		}
		else 
		{
...
// 				}
...
#endif
		{
			if(!m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Z1) || !m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Z2)) break;
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Z1, 0); // move up when tray 1 have tray 
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Z2, 1);
			m_nUnload1Case = 484;
		} else {
			if(!m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Z1) || !m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Z2)) break;
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Z1, 1); // move down
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Z2, 0); //move up when tray 2 have tray 
			m_nUnload1Case = 482;
		}
			m_pCommon->Set_LoopTime(AUTO_UNLOAD1, 30000);
		break;
	case 482:
		if (m_pCommon->Check_Position(AX_UNLOAD_TRAY_Z1, 1) && m_pCommon->Check_Position(AX_UNLOAD_TRAY_Z2, 0)) {
			if(!m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Y1) || !m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Y2)) break;
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Y1, 0); //load
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Y2, 3); //unload 
			m_nUnload1Case = 483;
			m_pCommon->Set_LoopTime(AUTO_UNLOAD1, 30000);
		}
		break;
	...
	case 490:
		if (m_pCommon->Check_Position(AX_UNLOAD_TRAY_Z1, 3)) {
			m_pDY6->oEnd_UnloadSupport12In = FALSE;
			m_pDY6->oEnd_UnloadSupport12Out = TRUE; // tray is there, then support out 
			m_pAJinAXL->Write_Output(6);
			m_sLog.Format("m_nUnload1Case,%d support Out",m_nUnload1Case); pLogFile->Save_MCCLog(m_sLog);
			m_nUnload1Case = 491;
			m_pCommon->Set_LoopTime(AUTO_UNLOAD1, 5000);
	...
	case 473:
		if (m_pCommon->Check_Position(AX_UNLOAD_TRAY_Y1, 3) && m_pCommon->Check_Position(AX_UNLOAD_TRAY_Y2, 0)) {
			if(!m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Z1) || !m_pAJinAXL->Is_Done(AX_UNLOAD_TRAY_Z2)) break;
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Z1, 1); // down
			m_pCommon->Move_Position(AX_UNLOAD_TRAY_Z2, 0); // tray up
			m_nUnload1Case = 474;
			m_pCommon->Set_LoopTime(AUTO_UNLOAD1, 30000);
		}
		break;
...
// 	if (m_pDX1->iLS_Load1FCheck && !m_pDX6->iEnd_Unload1FCheck) {
...
//				if (!m_pCommon->Delay_LoopTime(AUTO_UNLOAD2, 3000)) break;
	...
	case 200:
		if ((m_nUnload1Case < 192 || m_nUnload1Case > 411) && m_pCommon->Check_Position(AX_UNLOAD_TRAY_Y2, 1)) {
	...
	case 351:
		if (m_pDX6->iEnd_Unload1FCheck)
		{
			m_sLog.Format("m_nUnload2Case,%d, %d",m_nUnload2Case,m_pDX6->iEnd_Unload1FCheck); pLogFile->Save_MCCLog(m_sLog);
			break;
		}
		else 
		{
...
BOOL CSequenceMain::Inspect_Run()
{
	...
	case 130:	//검사완료까지 대기
		if (Check_IndexEmpty(2) == TRUE || Check_InspecDone()) {
			...
			m_nInspectCase = 150;
			gData.dwReloadStartTick = GetTickCount();
...
BOOL CSequenceMain::Barcode_Run()
{
	...
	case 120:
		...
		{
			m_sLog.Format("m_nBarcodeCase,%d, Back to Ready Pos Done",m_nBarcodeCase); pLogFile->Save_MCCLog(m_sLog);
...
//			m_pAJinAXL->Move_Abs_Accel(AX_BARCODE_A, m_dBarcodePosA, m_pMoveData->dDBarCodeA[0]);
	...
	case 200:
			m_sLog.Format("m_nBarcodeCase,%d, Back to Ready Pos Start",m_nBarcodeCase); pLogFile->Save_MCCLog(m_sLog);
			m_pCommon->Move_Position(AX_BARCODE_A, 0); //ready
			m_nBarcodeCase = 210;
			m_pCommon->Set_LoopTime(AUTO_BARCODE, 10000);
		break;
	case 210:
		if (m_pCommon->Check_Position(AX_BARCODE_A, 0)) //ready
		{
			m_sLog.Format("m_nBarcodeCase,%d, Back to Ready Pos Done",m_nBarcodeCase); pLogFile->Save_MCCLog(m_sLog);
			gData.IndexJob[2] = 1;
			m_nBarcodeCase = 100;
...
//		}
	...
	case 140:
		...
		{
			m_sLog.Format("m_nBarcodeCase,%d, Move to IndexDown Pos Start & Align Out",m_nNGPickerCase); pLogFile->Save_MCCLog(m_sLog);
	...
	case 162:
		...
		{
			if (!m_pCommon->Delay_LoopTime(AUTO_NGPICKER, 500)) break;
			m_sLog.Format("m_nBarcodeCase,%d, NG Picker Z Ready Up Start & Sylinder Up",m_nNGPickerCase); pLogFile->Save_MCCLog(m_sLog);
	...
	case 170:
		...
		{
			m_sLog.Format("m_nBarcodeCase,%d, NG Picker Z Ready Up Done & Sylinder Up",m_nNGPickerCase); pLogFile->Save_MCCLog(m_sLog);
			m_nNGPickerCase = 180;
			m_pCommon->Set_LoopTime(AUTO_NGPICKER, 5000);
		}
		break;
...
//		if((!m_pEquipData->bUseCMCheck) ||
	...
	case 210:	// NG Tray 위치로 이동 확인.
		if (/*m_pCommon->Check_Position(AX_NG_PICKER_X, 1) &&*/m_pCommon->Check_Position(AX_NG_PICKER_Z, 0) )
		{
...
BOOL CSequenceMain::ULPicker_Run()
{
	...
	case 140:
		...
		{
			if (!m_pCommon->Delay_LoopTime(AUTO_ULPICKER, 300)) break;
...
//			!m_pDX2->i_IndexAlignUnload_In && m_pDX2->i_IndexAlignUnload_Out ) {
	...
	case 310:
		...
		break;
	case 320:
		if (m_pCommon->Check_Position(AX_LOAD_PICKER_Z, 0) && (m_pCommon->Check_Position(AX_NG_PICKER_Z, 0) ) &&
...
BOOL CSequenceMain::Run_Simulation()
{
	...
	if(m_nUnload1Case == 100 || m_nUnload2Case == 100) m_pDX6->iEnd_Load1FCheck = TRUE;
