...
void CSetupMoveTab2Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LABEL_16, m_Label[16]);
	...
	DDX_Control(pDX, IDC_STC_COVER_2_Z_0, m_stcBarcodeA[0]);
	DDX_Control(pDX, IDC_STC_COVER_2_Z_1, m_stcBarcodeA[1]);
...
BEGIN_MESSAGE_MAP(CSetupMoveTab2Dlg, CDialogEx)
	...
	ON_CONTROL_RANGE(BN_CLICKED, IDC_STC_COVER_2_Z_0, IDC_STC_COVER_2_Z_1, OnStcBarcodeAClick)
...
void CSetupMoveTab2Dlg::OnStcBarcodeAClick(UINT nID)
{
 	int ID = nID - IDC_STC_COVER_2_Z_0;

	CCommon *pCommon = CCommon::Get_Instance();
	CString strOld, strNew;
	
	m_stcBarcodeA[ID].GetWindowText(strOld);
	if (pCommon->Show_NumPad(strOld, strNew) == IDOK) {
		m_stcBarcodeA[ID].SetWindowText(strNew);
	}
...
void CSetupMoveTab2Dlg::OnBnClickedBtnSave()
{
	...
	for (int i = 0; i < 2; i++) {
		m_stcBarcodeA[i].GetWindowText(strData);
		dData = atof(strData);
...
void CSetupMoveTab2Dlg::Initial_Controls() 
{
	...
	for (int i = 0; i < 17; i++) m_Label[i].Init_Ctrl("바탕", 11, FALSE, RGB(0xFF, 0xFF, 0xFF), RGB(0x60, 0x60, 0x60));

	for (int i = 0; i < 2; i++) m_stcCover1Z[i].Init_Ctrl("바탕", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0xB0, 0xF0, 0xB0));
	for (int i = 0; i < 2; i++) m_stcBarcodeA[i].Init_Ctrl("바탕", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0xB0, 0xF0, 0xB0));
...
void CSetupMoveTab2Dlg::Display_MoveData()
{
	...
	for (int i = 0; i < 2; i++) {
		...
		m_stcBarcodeA[i].SetWindowText(strData);
	}
